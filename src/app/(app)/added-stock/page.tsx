...
import { useStockAdditions, useCreateStockAddition, useDeleteStockAddition } from '@/hooks/useStockAdditions';
...
export default function AddedStockPage() {
  ...
  const { data: additions, isLoading } = useStockAdditions({
    additionDate: selectedDate,
    shiftType: selectedShift,
    storeLocation: selectedStore,
  });

  const createMutation = useCreateStockAddition();
  const deleteMutation = useDeleteStockAddition();

  const filteredAdditions = (additions || []).filter((a) => a.stock_type === activeTab);

  const options = activeTab === 'value' ? VALUE_OPTIONS : DRAWER_OPTIONS;

  // Reset newItemKey when switching tabs
  ...
  const handleAdd = async () => {
    try {
      ...
      setNewQuantity(1);
      setNewNotes('');
      toast.success('Stock addition saved');
    } catch {
      toast.error('Failed to save addition');
    }
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      toast.success('Addition removed');
    } catch {
      toast.error('Failed to remove addition');
    }
  };

  const totalQty = filteredAdditions.reduce((sum, a) => sum + a.quantity, 0);
  const valueCount = (additions || []).filter((a) => a.stock_type === 'value').length;
  const drawerCount = (additions || []).filter((a) => a.stock_type === 'drawer').length;

  return (
    <div className="space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Added Stock</h1>
        <p className="text-sm text-gray-500 mt-1">
          Record stock received during a shift. These additions automatically update shift report counts.
        </p>
      </div>
      ...
      <div className="card">
        ...
        <div className="px-6 pt-5 flex gap-2">
          ...
          >
            ...
            {valueCount > 0 && (
              <span className={[
                ...
                activeTab === 'value' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-700',
              ].join(' ')}>
                {valueCount}
              </span>
            )}
          ...
          >
            ...
            {drawerCount > 0 && (
              <span className={[
                'ml-1 px-1.5 py-0.5 rounded-full text-xs font-semibold',
                activeTab === 'drawer' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-700',
              ].join(' ')}>
                {drawerCount}
              </span>
            )}
        ...
        <div className="p-6">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p className="text-sm">No {activeTab === 'value' ? 'value stock' : 'drawer stock'} additions for this shift.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredAdditions.map((addition) => (
                    <tr key={addition.id} className="hover:bg-blue-50/30 transition-colors">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{addition.item_key}</td>
                      <td className="px-4 py-3 text-sm text-center font-semibold text-gray-700">{addition.quantity}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{addition.notes || '-'}</td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-200 bg-gray-50/80">
                    ...
                    <td className="px-4 py-3 text-sm text-center font-bold text-gray-900">{totalQty}</td>
        ...
        <div className="px-6 pb-6 border-t border-gray-200 pt-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Add New</h3>
